...
ResponseBody expired() => response(401, {
  'error': {'code': 'SESSION_EXPIRED', 'message': 'private server detail'},
});
...
void main() {
  test(
    'injected Dio retains adapter and receives base URL and timeouts',
    () async {
      final dio = Dio();
    ...
    final dio = Dio()
      ..httpClientAdapter = Adapter((request) async {
        if (request.path.endsWith('/refresh')) {
          refreshes++;
          await gate.future;
          return response(200, session('new').toJson());
        }
  ...
  test(
    'authoritative refresh rejection clears storage and signals auth once',
    () async {
      final store = MemorySessionStore();
      var expiredEvents = 0;
  ...
  test(
    'temporary refresh failure preserves persisted recovery material',
    () async {
      final store = MemorySessionStore();
      final dio = Dio()
        ..httpClientAdapter = Adapter(
          (request) async =>
              request.path.endsWith('/refresh') ? response(503, {}) : expired(),
        );
      final client = DioApiClient(
        baseUri: Uri.parse('https://api.example.test'),
        sessionStore: store,
  ...
  test(
    'reads retry transient errors but mutations are never blindly replayed',
    () async {
      var gets = 0;
      var posts = 0;
      final dio = Dio()
        ..httpClientAdapter = Adapter((request) async {
          ...
          return response(503, {
            'error': {'message': 'SQL password=secret', 'code': 42},
          });
        });
      final client = DioApiClient(
        baseUri: Uri.parse('https://api.example.test'),
